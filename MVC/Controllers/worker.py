...
class WorkerController:
    ...
    def get_info(self):
        for x in self.model.get_worker_info(self.worker.id):
            ...
            self.worker.name_text.insert(1.0, f"{x[1]} {x[2]} {x[3]}")
            self.worker.email_text.insert(1.0, f"{x[4]}")
            self.worker.birth_date_text.insert(1.0, f"{x[5]}")
            self.worker.post_text.insert(1.0, f"{x[6]}")
